<?php
/**
 * Plugin Name: Marriland IV Calculator
 * Plugin URI: https://marriland.com
 * Description: Plugin to Operate IV Calculation Procedures
 * Version: 0.1.2
...
define('MARRILAND_IV_CALCULATOR_PLUGIN_PATH', plugin_dir_path(__FILE__));
define('MARRILAND_IV_CALCULATOR_PLUGIN_URL', plugins_url(__FILE__));
...
Utility::loader([
	'main.class.php',
	'iv-calculator.class.php',
	'hidden-power-ivs.class.php'
...
//# that works.
function marriland_iv_calculator_init() {
	//! v-- This removes weird <p> tags from being inserted in HTML! Important!
	...
	//# IMPORTANT: Uncomment the below bit after changing the PLUGIN part.
		MARRILAND_IV_CALCULATOR_PAGE_ID
	];
	...
	$loader = new \Twig\Loader\FilesystemLoader(MARRILAND_IV_CALCULATOR_PLUGIN_PATH . 'views');
	...
	$calculator = (new \Marriland\IV_Calculator\Main())
		->use_twig($twig)
		->use_db($db);
	$calculator->init();
}
...
add_action('wp_head', 'marriland_iv_calculator_init', 0);
/** */

function ajax_marriland_iv_calculator() {
	$db = Utility::connect('pokedex');

	$pokemon = (new \Marriland\IV_Calculator\Main(['name' => $_GET['name']]))->use_db($db);
	$pokemon->init();
	$json = $pokemon->ajax();
	wp_die($json);
}
add_action('wp_ajax_marriland_iv_calculator', 'ajax_marriland_iv_calculator');
add_action('wp_ajax_nopriv_marriland_iv_calculator', 'ajax_marriland_iv_calculator');