<?php
/**
 * Plugin Name: Marriland EXP Calculator
 * Plugin URI: https://marriland.com/
 * Description: EXP Calculating Plug-in
 * Version: 0.1.2
...
define('MARRILAND_EXP_CALCULATOR_PLUGIN_PATH', plugin_dir_path(__FILE__));
define('MARRILAND_EXP_CALCULATOR_PLUGIN_URL', plugins_url(__FILE__));
...
//# that works.
function marriland_exp_calculator_init() {
	...
	//# IMPORTANT: Uncomment the below bit after changing the PLUGIN part.
		MARRILAND_EXP_CALCULATOR_PAGE_ID
	];
	...
	$loader = new \Twig\Loader\FilesystemLoader(MARRILAND_EXP_CALCULATOR_PLUGIN_PATH . 'views');
	...
	$plugin = (new \Marriland\EXP_Calculator\Main())
		->use_twig($twig)
		->use_db($db);
	$plugin->init();
}
...
add_action('wp_head', 'marriland_exp_calculator_init', 0);
/** */

function ajax_marriland_exp_calculator() {
	//# Connect to the MongoDB and the `pokedex` DB.
	$db = Utility::connect('pokedex');
...
add_action('wp_ajax_marriland_exp_calculator', 'ajax_marriland_exp_calculator');
add_action('wp_ajax_nopriv_marriland_exp_calculator', 'ajax_marriland_exp_calculator');